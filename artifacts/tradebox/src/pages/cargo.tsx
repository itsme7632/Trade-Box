      ...
      <div className="max-w-7xl mx-auto w-full space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[#1E293B] pb-6">
          <div>
            ...
            <p className="text-gray-400 font-mono text-sm mt-1 uppercase">Active and completed investments</p>
          </div>
        </div>
        ...
        <Tabs defaultValue="all" onValueChange={(v) => setStatusFilter(v as ListInvestmentsStatus)}>
          ...
          <div className="space-y-4">
            {isLoading ? (
              [...Array(3)].map((_, i) => <Skeleton key={i} className="h-40 w-full bg-[#1E293B] rounded-xl" />)
            ) : filtered && filtered.length > 0 ? (
              filtered.map(inv => (
                <div key={inv.id} className="bg-[#1E293B] rounded-xl border border-[#334155] p-0 overflow-hidden hover:border-[#0066FF]/50 transition-colors">
                  <div className="p-4 md:p-6 flex flex-col lg:flex-row gap-6 items-start lg:items-center">
                    ...
                    <div className="flex-1 w-full">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-gray-400 font-mono text-xs uppercase">{inv.shipment?.cargoType}</span>
                      </div>
                      <Link href={`/market/shipments/${inv.shipmentId}`} className="text-xl font-heading font-bold text-white hover:text-[#0066FF] transition-colors line-clamp-1 block mb-2">
                        {inv.shipment?.title}
                      </Link>
                    ...
                    <div className="flex-1 w-full lg:w-auto grid grid-cols-2 md:grid-cols-4 gap-4 bg-[#0F1923] p-4 rounded-lg border border-[#334155]/50">
                      <div>
                        <p className="text-[10px] text-gray-500 font-mono uppercase tracking-wider mb-1">Committed</p>
                        <p className="font-bold font-mono text-white">${inv.amount.toLocaleString()}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-gray-500 font-mono uppercase tracking-wider mb-1">Return</p>
                        <p className="font-bold font-mono text-[#0066FF]">+{inv.profitPercent}%</p>
                      </div>
                      <div className="col-span-2 md:col-span-2 border-t md:border-t-0 md:border-l border-[#334155] pt-3 md:pt-0 md:pl-4">
                        ...
                        <p className={`font-bold font-mono text-xl ${inv.status === 'delivered' ? 'text-[#22C55E]' : 'text-white'}`}>
                  ...
                  {inv.status === 'active' && inv.shipment && (
                    <div className="bg-[#0066FF]/5 px-6 py-3 border-t border-[#0066FF]/20 flex justify-between items-center">
                      ...
                      <Link href="/tracker" className="text-xs font-bold text-white hover:text-[#0066FF] font-mono uppercase underline-offset-4 hover:underline">
                        Track Vessel
                      </Link>
                    </div>
                  )}
                  {inv.status === 'delivered' && (
                    <div className="bg-[#22C55E]/5 px-6 py-3 border-t border-[#22C55E]/20 flex justify-between items-center">
                      ...
                      <span className="text-xs font-bold text-white font-mono uppercase">Profit Credited to Wallet</span>
                    </div>
                  )}
                </div>
              ))
              ...
              <div className="flex flex-col items-center justify-center py-20 bg-[#1E293B] rounded-xl border border-[#334155] border-dashed">
                <Ship className="h-12 w-12 text-gray-500 mb-4 opacity-50" />
