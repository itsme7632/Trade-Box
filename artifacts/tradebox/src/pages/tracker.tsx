...
import { Ship, Anchor, Activity, Clock } from "lucide-react";
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-2 bg-[#1E293B] rounded-xl border border-[#334155] overflow-hidden flex flex-col">
            ...
            <div className="relative flex-1 min-h-[400px] bg-[#0A1017] p-8 flex items-center justify-center overflow-hidden">
              ...
              <svg viewBox="0 0 1000 500" className="absolute inset-0 w-full h-full opacity-20 pointer-events-none stroke-[#334155]" fill="none" strokeWidth="1">
                {/* Simplified continent outlines */}
                <path d="M 200,100 Q 250,50 300,80 T 350,150 Q 300,250 250,300 T 150,400 Q 100,300 150,200 Z" /> {/* Americas rough */}
                <path d="M 450,100 Q 550,50 650,100 T 700,200 Q 600,300 500,250 T 450,150 Z" /> {/* Eurasia/Africa rough */}
                <path d="M 750,300 Q 800,250 850,300 T 900,400 Q 800,450 750,400 Z" /> {/* Oceania rough */}
              </svg>

              {/* Grid lines */}
              <div className="absolute inset-0" style={{ backgroundImage: 'linear-gradient(#1E293B 1px, transparent 1px), linear-gradient(90deg, #1E293B 1px, transparent 1px)', backgroundSize: '50px 50px', opacity: 0.2 }} />
                ...
                <div className="relative w-full max-w-4xl h-full py-20 flex flex-col justify-around gap-8 z-10">
                  {shipments.map((s, i) => (
                    <div key={s.id} className="relative w-full">
                      ...
                      {/* Track Line */}
                      <div className="h-1 w-full bg-[#1E293B] relative rounded-full overflow-hidden border border-[#334155]">
                        <div 
                      ...
                      {/* Vessel Marker */}
                      <div 
                        className="absolute top-6 -translate-y-1/2 -ml-3 z-20 group cursor-pointer"
                        style={{ left: `${s.progressPercent}%` }}
                      >
                        <div className="bg-[#0066FF] p-1.5 rounded-full shadow-[0_0_10px_#0066FF]">
                        ...
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-[#1E293B] border border-[#334155] p-2 rounded shadow-xl opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-30">
                          ...
                          <p className="text-[10px] text-gray-400 font-mono mt-0.5">ETA: {s.etaDays} days</p>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              {isActivitiesLoading ? (
                ...
                activities.map(act => (
                  <div key={act.id} className="flex gap-3 border-b border-[#334155]/50 pb-4 last:border-0">
                    <div className={`mt-0.5 p-1.5 rounded bg-[#0F1923] border shrink-0
                      ${act.eventType === 'arrival' ? 'border-[#22C55E] text-[#22C55E]' : 'border-[#0066FF] text-[#0066FF]'}`}>
                      {act.eventType === 'arrival' ? <Anchor className="h-4 w-4" /> : <Ship className="h-4 w-4" />}
                    </div>
                    <div>
                      <p className="text-sm">
                        <span className="font-bold text-white">{act.vesselName}</span>
                        <span className="text-gray-400"> {act.eventType === 'arrival' ? 'arrived at' : 'departed from'} </span>
                        <span className="font-bold text-white">{act.portName}, {act.country}</span>
                      </p>
                      <div className="flex items-center gap-3 mt-1 text-xs text-gray-500 font-mono">
                        <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {format(parseISO(act.timestamp), 'HH:mm')}</span>
                        {act.cargoType && <span className="uppercase text-[#0066FF] border border-[#0066FF]/30 px-1 rounded">{act.cargoType}</span>}
                      </div>
                    </div>
                  </div>
                ))
              ) : (
                <div className="text-center text-gray-500 font-mono py-10">No recent port activity.</div>
              )}
            </div>
          </div>
